...
import com.example.myweiyue.BuildConfig;
import com.example.myweiyue.app.Constants;
import com.example.myweiyue.app.MyApp;
import com.example.myweiyue.utils.NetUtil;
...
public class RetrofitConfig {
    ...
    public static final Interceptor sRewriteCacheControlInterceptor = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            Response originalRequest = chain.proceed(request);
            if (NetUtil.isNetworkAvailable(MyApp.getContext())) {
                //有网的时候读接口上的@Headers里的配置，你可以在这里进行统一的设置
                String cacheControl = request.cacheControl().toString();
                return originalRequest.newBuilder().header("Cache-Control", cacheControl)
                        .removeHeader("Pragma").build();
            } else {
                return originalRequest.newBuilder().header("Cache-Control", "public," + CACHE_CONTROL_CACHE)
                        .removeHeader("Pragma").build();
            }
        }
    };
    ...
    public static final HttpLoggingInterceptor sLoggingInterceptor() {
